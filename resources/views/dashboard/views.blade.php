  ...
  <div class="space-y-4 p-4">
    ...
    <div class="w-full flex flex-col gap-4">
      ...
      <div class="p-2 w-full bg-neutral-800 rounded flex xl:flex-row flex-col gap-2">
        ...
        <form action="{{route('dashboard.'.$current.'.list', array_replace($filters, array('range' => 'range')))}}" method="get" class="flex gap-2">
          <input type="hidden" value="range" name="range">
          <div class="flex justify-center items-center rounded {{$filters['range'] == 'range' ? 'border-2 border-blue-500' : ''}}">
            <span class="bg-neutral-700 px-4 py-2 rounded-l h-full">Start</span>
            <input type="date" name="start" class="px-4 py-2 bg-neutral-700 hover:bg-neutral-600 hover:text-white rounded-r rounded-l-none h-full" value="{{$filters['start']}}">
          </div>
          <div class="flex justify-center items-center rounded {{$filters['range'] == 'range' ? 'border-2 border-blue-500' : ''}}">
            <span class="bg-neutral-700 px-4 py-2 rounded-l h-full">End</span>
            <input type="date" name="end" class="px-4 py-2 bg-neutral-700 hover:bg-neutral-600 hover:text-white rounded-r rounded-l-none h-full" value="{{$filters['end']}}">
          </div>
          <input type="submit" class="px-4 py-2 bg-neutral-700 hover:bg-neutral-600 hover:text-white rounded cursor-pointer" value="Filter">
        </form>
      </div>
